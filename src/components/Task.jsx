...
function Task({ taskName, taskIndex, taskIsActive, secondsLeft, setIsActive, setIsStopped, taskQueue, setTasks }) {
    ...
    const handleDelete = () => {
        ...
        setTasks([...taskQueue.slice(0, taskIndex), ...taskQueue.slice(taskIndex + 1)])
    }
    ...
    const handleSubmitEdit = (event) => {
        ...
        setTasks([...taskQueue.slice(0, taskIndex), task, ...taskQueue.slice(taskIndex + 1)]);
        setIsEditing(false);
    }
    ...
    if (!isEditing) {
        viewData = <div className={`${styles.task} ${taskIsActive ? styles.active : ''}`}>
